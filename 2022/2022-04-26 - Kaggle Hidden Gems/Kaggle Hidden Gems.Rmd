...
twitter_profiles <- hidden_gems %>%
  filter(
    !is.na(author_twitter),
    !str_detect(author_twitter, "linkedin")
  ) %>%
  mutate(
    author_twitter = ifelse(
      str_detect(author_twitter, "twitter"),
      author_twitter,
      paste0("https://twitter.com/", author_twitter)
    )
  ) %>%
  pull(author_twitter)

account_does_not_exist <- "https://twitter.com/CaptCalculator"
problem_profile <- 'https://twitter.com/jiweiliu'
```

```{r}
# twitter values
does_not_exist_xpath <- '//*[@id="react-root"]/div/div/div[2]/main/div/div/div/div/div/div[2]/div/div/div[2]'
followers_element <- '//*[@id="react-root"]/div/div/div[2]/main/div/div/div/div[1]/div/div[2]/div/div/div/div/div[5]/div[2]'
```

```{r}
remDr$open()
remDr$navigate(problem_profile)
try(remDr$findElement(using = 'xpath', followers_element))
```
...
for (profile in twitter_profiles) {
  ...
  if (!is.na(profile)) {
    ...
    exists_test <- try(remDr$findElement(using = "xpath", does_not_exist_xpath)$getElementText())
    if (str_detect(exists_test, "This account doesn’t exist\nTry searching for another.") == TRUE) {
      followers <- 'Page Not Found'
    } else {
      ## retrieve number of twitter followers
      wetest(using = "xpath", element = followers_element, sleepmin = 0.5, sleepmax = 1)
      followers_xpath <- try(remDr$findElement(using = "xpath", followers_element))
      if (str_detect(followers_xpath$getElementText(), "Followers") == TRUE) {
        followers <- followers_xpath$getElementText()
      } else {
        followers <- 'NULL'
      }
...
sel_twitter <- selenium_df %>%
  as.data.frame() %>%
  mutate(
    author_twitter = unlist(author_twitter),
