...
remDr$findElement(using = 'xpath', page_not_found)$getElementText()
```
...
head(list(hidden_gems$author_twitter)[[1]], 5)
```
...
selenium_df <- NULL
#for (row in 1:nrow(hidden_gems)) {
for (tweet in head(list(hidden_gems$author_twitter)[[1]], 2)) {
  remDr$open()
  ...
  if (!is.na(tweet)) {
    ...
    remDr$navigate(tweet)
    wetest(using = twitter_loaded, element = '//*[@id="react-root"]/div/div/div[2]/header/div/div/div/div', sleepmin = 0.5, sleepmax = 1)
...
#colnames(selenium_df) <- c("author_twitter", "notebook", "followers")
#selenium_df
