...
image_labels <- c(
  ...
  "Aye-Aye" = glue("<img src='{here('2021', '20210824 - Lemurs', 'images', 'I - Aye-Aye.png')}' width='80'/><br>*Aye-Ayer*")
  )
```
...
```{r Avg Litter Size, fig.height=16, fig.width=4}
# average litter size by taxon
## colored by gender (may have to intuite this by distribution of gender)
## maybe use geom_jitter() for this?
lemurs %>%
  inner_join(taxon_map, by = "taxon") %>%
  group_by(taxon, common_name) %>%
  dplyr::summarise(
    avg_litter_size = mean(litter_size, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  ggplot(aes(avg_litter_size, common_name)) + 
  geom_text(aes(label = round(avg_litter_size)), size = 5) + 
  facet_wrap(~ common_name, ncol = 1, scales = "free") + 
  scale_y_discrete(name = NULL, labels = image_labels) +
  theme(
    axis.text.y = element_markdown(margin = margin(b = 8))
  )
```

```{r Lemur Growth Rates, fig.height=16, fig.width=4}
